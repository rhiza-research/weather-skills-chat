...
from open_webui.utils.skills import set_skill_active, set_skill_enabled
...
class SkillEnabledToggleTest(unittest.TestCase):
    def test_set_skill_enabled_updates_pack_and_tool_manifest(self):
        ...
        with (
            ...
            self.assertFalse(args[1]["meta"]["skills"][0]["enabled_by_default"])
    ...
    def test_set_skill_active_updates_pack_meta(self):
        pack = SkillPackModel(
            id="pack-1",
            user_id="user-1",
            name="demo@main",
        ...
        tool.meta.model_dump.return_value = {
            "description": "d",
            "manifest": {"kind": "skill", "pack_id": "pack-1", "is_active": True},
        }
        ...
        with (
            ...
            result = set_skill_active("pack-1", "tool_plot", False)

            args = packs.update.call_args[0]
            self.assertFalse(args[1]["meta"]["skills"][0]["is_active"])
            tool_update = tools.update_tool_by_id.call_args[0][1]
            self.assertFalse(tool_update["meta"]["manifest"]["is_active"])
            self.assertIs(result, updated)
