...
def _openai_headers(url: Optional[str], key: Optional[str], user: UserModel = None):
    headers = {"Content-Type": "application/json"}
    if key:
        headers["Authorization"] = f"Bearer {key}"
        # Anthropic's GET /v1/models is the native API and rejects Bearer.
        if url and "anthropic.com" in url:
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                url,
                headers=_openai_headers(url, key, user),
            ) as response:
